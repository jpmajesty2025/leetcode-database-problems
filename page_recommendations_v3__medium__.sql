...
-- The Pure JOIN + Anti-Join Method (Optimized for Join-Heavy Execution Planners)
-- Uses a CTE to capture all friends, joins to Likes for candidate pages, and uses a
-- LEFT JOIN on Likes for user 1 where page_id IS NULL to filter out already-liked pages.
-- Relies entirely on relational join operations (Hash Join + Hash Anti-Join) rather than subqueries.
